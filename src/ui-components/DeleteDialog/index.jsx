    ...
    <Dialog.Root open={isOpen} onOpenChange={(open) => { if (!open) handleClose()}}>
        <Dialog.Portal>
            ...
            <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white dark:bg-gray-900 p-6 rounded shadow-lg">
                <Dialog.Title className="text-lg font-bold dark:text-gray-200">Confirm Deletion</Dialog.Title>
                <Dialog.Description className="mt-2 text-sm text-gray-600 dark:text-gray-200">
                    {prompt}
                </Dialog.Description>
                <div className="mt-4 flex justify-end space-x-2">
                    <MyButton color="mute" onClick={cancel}>No, Cancel</MyButton>
                    <MyButton color="danger" onClick={() => proceed(true)}>Yes, delete</MyButton>
                </div>
