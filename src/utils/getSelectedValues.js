const getSelectedValues = (value, options) =>{
  ...
  const getSelectValue = (value, options) =>{
    ...
    if (options && options.length > 0){
        if(typeof value ===  "object"){
          if (options.includes(value)){
            ...
            return options.find((o) => o.value === value?.value || o.value === value?.id || o.value === value?.serial);
          }
        } else {
    ...
    else if (typeof value === 'object' && (value?.title || value?.name || value?.label) && (value?.id || value?.serial)) {
        ...
        return { value: value?.id || value?.serial, label: value.title || value?.name || value?.label};
    }
